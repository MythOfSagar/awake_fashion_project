...
const [quanityPrice,setquanityPrice]=useState(
  basketData.map((el)=>{return {price:el.price,qty:1,id:el.id}})
)
...
let total=0
  let numP=0
  for(let v=0;v<(arr.map((el)=>{
    if(obb.id===el.id){return obb}
    else {return el}
  }
  )).length;v++){
     total=total+(arr.map((el)=>{
      if(obb.id===el.id){return obb}
      else {return el}
    }
    ))[v].price*(arr.map((el)=>{
  ...
  return (
    ...
    padding={{ base: '15px', md: '30px', lg: '56px' }}>
      ...
      backgroundColor={"white"}>
        <Heading fontSize={{ base: '22px', md: '30px', lg: '30px' }}>Your Shopping Basket</Heading>
        <Heading fontSize={{ base: '20px', md: '27px', lg: '27px' }}>{numberOfProducts} Products</Heading>
      </Box>
      <Box display="flex">
    <Box
    ...
    backgroundColor={"white"}>
      <Box textAlign={"center"}>
        <Text
        fontSize={{ base: '14', md: '19', lg: '56px' }}
        >Total</Text>
